...
from models.modules.losses.ssim2 import SSIM, MS_SSIM
    ...
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if train_opt['hfen_weight']:
                ...
                if l_hfen_type:
                    self.cri_hfen = HFENLoss(
                        loss_f=l_hfen_type, device=self.device, pre_smooth=pre_smooth, relative=relative).to(self.device)
                else:
                    raise NotImplementedError(
            ...
            if train_opt['tv_weight']:
                ...
                if l_tv_type == 'normal':
                    self.cri_tv = TVLoss(
                        self.l_tv_w, p=tv_norm).to(self.device)
                elif l_tv_type == '4D':
            ...
            if train_opt['ssim_weight']:
                ...
                if l_ssim_type == 'ssim':
                    self.cri_ssim = SSIM(
                        win_size=11, win_sigma=1.5, size_average=True, data_range=1., channel=3).to(self.device)
            ...
            if train_opt['lpips_weight']:
                ...
                self.cri_lpips = models.PerceptualLoss(
                    model=lpips_type, net=lpips_net, use_gpu=lpips_GPU, model_path=None, spatial=lpips_spatial)  # .to(self.device)
            ...
            if train_opt['spl_weight']:
                ...
                if l_spl_type == 'spl':  # Both GPL and CPL
                    ...
                    self.cri_cpl = spl.CPLoss(
                        rgb=True, yuv=True, yuvgrad=True, spl_norm=self.spl_norm, yuv_norm=self.yuv_norm)
    ...
    def optimize_parameters(self, step):
        ...
        if self.cri_gan:
            if step % self.D_update_ratio == 0 and step > self.D_init_iters:
                if self.cri_pix:  # pixel loss
                    l_g_pix = self.l_pix_w * \
                        self.cri_pix(self.fake_H, self.var_H)
                    l_g_total += l_g_pix
                if self.cri_ssim:  # structural loss
                    # using ssim2.py
                    l_g_ssim = 1.-(self.l_ssim_w *
                                   self.cri_ssim(self.fake_H, self.var_H))
                ...
                if self.cri_hfen:  # HFEN loss
                    l_g_HFEN = self.l_hfen_w * \
                        self.cri_hfen(self.fake_H, self.var_H)
                    l_g_total += l_g_HFEN
                ...
                if self.cri_lpips:  # LPIPS loss
                    ...
                    l_g_lpips = self.cri_lpips.forward(
                        self.fake_H, self.var_H, normalize=self.lpips_norm).mean()
                ...
                if self.cri_gpl:  # GPL Loss (SPL)
                    l_g_gpl = self.l_spl_w * \
                        self.cri_gpl(self.fake_H, self.var_H)
                    #l_spl = l_g_gpl + l_g_cpl
                    l_g_total += l_g_gpl
                if self.cri_cpl:  # CPL Loss (SPL)
                    l_g_cpl = self.l_spl_w * \
                        self.cri_cpl(self.fake_H, self.var_H)
                    l_g_total += l_g_cpl
            ...
            if self.opt['train']['gan_type'] == 'wgan-gp':
                ...
                interp = self.random_pt * self.fake_H.detach() + (1 - self.random_pt) * \
                    self.var_ref
            ...
            if self.cri_ssim:  # structural loss (Structural Dissimilarity)
                ...
                l_g_ssim = 1.-(self.l_ssim_w *
                               self.cri_ssim(self.fake_H, self.var_H))
                # at random, l_g_ssim is returning NaN for ms-ssim, which breaks the model. Temporary hack, until I find out what's going on.
                if torch.isnan(l_g_ssim).any():
                    l_g_total = l_g_total
                else:
                    l_g_total += l_g_ssim
