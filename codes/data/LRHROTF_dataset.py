    ...
    def __init__(self, opt):
        ...
        if self.paths_LR and self.paths_HR:
            ...
            if len(self.paths_LR) < len(self.paths_HR):
                print('LR contains less images than HR dataset  - {}, {}. Will generate missing images on the fly.'.format(
                    len(self.paths_LR), len(self.paths_HR)))
                i = 0
                tmp = []
    ...
    def __getitem__(self, index):
        ...
        if self.opt['color_LR'] or self.opt['color']:  # Only change LR
            img_LR = util.channel_convert(
                img_LR.shape[2], self.opt['color'], [img_LR])[0]
        ...
        if self.opt['phase'] == 'train':
            ...
            if img_HR.shape[0]//img_LR.shape[0] != img_HR.shape[1]//img_LR.shape[1]:
                print(
                    "Warning: img_LR dimensions ratio does not match img_HR dimensions ratio for: ", HR_path)
                img_LR = img_HR
            ...
            if img_HR.shape[0] < HR_size or img_HR.shape[1] < HR_size:
                print("Warning: Image: ", HR_path, " size does not match HR size: (",
            ...
            if img_HR.shape[0] != HR_size or img_HR.shape[1] != HR_size:
                ...
                if self.opt['lr_downscale_types']:
                    ...
                    ds_algo = 777
                img_LR, _ = augmentations.scale_img(
                    img_HR, scale, algo=ds_algo)
            ...
            if self.opt['lr_cutout'] and (self.opt['lr_erasing'] != True):
                ...
                if np.random.rand() > 0.5:  # only do cutout or erasing, not both at the same time
                    img_LR = augmentations.cutout(
                        img_LR, img_LR.shape[0] // 2, p=0.5)
                else:
                    img_LR = augmentations.random_erasing(
                        img_LR, p=0.5, modes=[3])
            # """
            ...
            if self.opt['auto_levels'] and np.random.rand() > rand_levels:
                if self.opt['auto_levels'] == 'HR':
                    img_HR = augmentations.simplest_cb(
                        img_HR, znorm=self.znorm)
                elif self.opt['auto_levels'] == 'LR':
                    img_LR = augmentations.simplest_cb(
                        img_LR, znorm=self.znorm)
